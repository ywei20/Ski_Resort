...
public class Phase {
  ...
  public Phase(int phaseName, int numRunningThreads, int numSkiers, int numRuns, int numLifts,
    ...
    this.latchBefNextPhase = (phaseName != 3)? new CountDownLatch((int) (numRunningThreads * 0.1)) : null;
  }
  ...
  public int calNumReqPerThread() {
    if (phaseName == 3) {
      return (int) (numRuns * numRunsRatio);
    } else {
      return (int) (numRuns * numRunsRatio * numSkiers / numRunningThreads);
    }
  }
...
//      System.out.println("skierStartId: " + skierStartId + " skierEndId: " + skierEndId);
      ...
      PostThread pthread = new PostThread(skierStartId, skierEndId, startTime, endTime, numLifts,
          calNumReqPerThread(), successReqCnt, failedReqCnt, latchBefNextPhase, latchAllPhase, serverAddress);
